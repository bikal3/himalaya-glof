...
# ══════════════════════════════════════════════════════════════════════════════
st.subheader("4. Lake Area Cache (2016–2024)")
st.markdown(
    "One JSON file per lake, bundled as a zip. Each file records its own `source`: files written "
    "by `data/fetch_sentinel.py` hold MNDWI-derived Sentinel-2 measurements, while the cache "
    "shipped here is generated by `data/create_demo_cache.py` from the simulated area series."
)
st.caption("Schema per file: `lake_id · lake_name · last_updated · source · "
           "scenes[ year · date · area_km2 · cloud_pct ]`")
...
st.markdown(
    "Confirmed GLOF events used to train the Random Forest classifier, manually curated from the "
    "ICIMOD GLOF database. Coverage is the wider Hindu Kush Himalaya — Nepal, Bhutan, Tibet and "
    "Sikkim — not Nepal alone. The events are real; the per-event slope, settlement distance and "
    "growth-rate attributes are unverified estimates."
)
...
st.markdown(
    ...
    "catalogue. Features: area, growth rate, dam type, slope, distance to settlement, elevation. "
    "Cross-validation scores are omitted deliberately: the positive and negative classes come "
    "from separately authored files, so any score reflects that split rather than predictive "
    "skill — see the ML Risk Scoring page."
)
st.caption(
    "⚠️ This is a pickled (joblib) object. Loading a pickle executes code — only load it if "
    "you trust this source, or retrain your own with `python data/train_model.py`."
)
